...
class Lead_model extends MY_Model 
{
	...
	{  
        $this->db->select($this->select_column)	
            ->from($this->table)
            ->where(['l.is_deleted' => 0, 'l.is_archived' => $this->input->post('archive'), 'l.is_new' => $this->input->post('new_lead'), 'client_type' => 'Lead'])
            ->join('follow_ups f', 'f.lead_id = l.id', 'left')
            ->join('country c', 'c.id = l.inquiry_country')
            ->join('admins a', 'a.id = l.assigned', 'left')
            ->group_by('l.id');

        if (in_array($this->role, ['LMS Employee']))
            $this->db->where(["l.assigned" => $this->id]);
        ...
        if ($this->input->post('lmsemp'))
            $this->db->where(["f.created_by" => d_id($this->input->post('lmsemp'))]);
    ...
    public function count()
    {
        $this->db->select('l.id')
                ->from($this->table)
                ->where(['l.is_deleted' => 0, 'l.is_archived' => $this->input->post('archive') ? $this->input->post('archive') : 0, 'l.is_new' => $this->input->post('new_lead') ? $this->input->post('new_lead') : 0, 'client_type' => 'Lead'])
                ->join('follow_ups f', 'f.lead_id = l.id', 'left')
                ->group_by('l.id');
