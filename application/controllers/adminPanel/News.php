...
class News extends MY_Controller {
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('news_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('news_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function edit($id, $data=[])
    {
        ...
        $data['url'] = $this->redirect;
        $data['data'] = $this->main->get($this->table, 'news, image', ['id' => d_id($id)]);
	...
	public function uploadImage()
	{
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (!$this->upload->do_upload("image"))
                ...
                if ($img && $img != "No image" && file_exists($config['upload_path'].$img))
                    unlink($config['upload_path'].$img);
                return ['error' => false, 'img' => $this->upload->data("file_name")];
        }else
            if ($img)
                return ['error' => false, 'img' => $img];
            else
                return ['error' => false, 'img' => "No image"];
	}
